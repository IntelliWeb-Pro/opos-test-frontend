// src/app/oposicion/[id]/page.js (CORREGIDO)

'use client';

import { useState, useEffect } from 'react';
import { useParams } from 'next/navigation'; // <-- 1. IMPORTAMOS EL HOOK

export default function OposicionPage() { // <-- 2. QUITAMOS { params } DE AQUÍ
  ...
  useEffect(() => {
    ...
    fetch(`https://opos-test-backend.onrender.com/api/oposiciones/${params.id}/`)
      .then(response => {
        if (!response.ok) {
    ...
    <main className="bg-slate-100 min-h-screen p-8">
      <div className="container mx-auto">
        <header className="mb-12">
          <a href="/" className="text-blue-600 hover:underline">← Volver a todas las oposiciones</a>
        ...
        <div className="bg-white p-8 rounded-lg shadow-md">
          <ul className="space-y-4">
            {oposicion.temas.map(tema => (
              <li key={tema.id}>
                <a href={`/tema/${tema.id}`} className="block p-4 rounded-md hover:bg-slate-50 transition-colors duration-200">
